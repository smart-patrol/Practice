#http://www.r-bloggers.com/r-how-to-layout-and-design-an-infographic/
...
p1 <- ggplot(data = dat, aes(x = reorder(x, y1) , y = y1)) + geom_bar(stat = "identity", file = "#552683") +
...
# configure theme for line chart
...
#----------------------------------------------------------------------
# now place the plots into the layout
library(grid)

pdf("~/Infographics1.png", width = 10, height = 20)

grid.newpage()  # create new page

pushViewport(viewport(layout = grid.layout(4, 3))) # set the layout dimensions

grid.rect(gp = gpar(fill = "#E2E2E3", col = "#E2E2E3")) # set background colour
